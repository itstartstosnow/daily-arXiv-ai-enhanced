...
TL;DR: 首次对React2Shell漏洞（CVE-2025-55182）进行互联网规模测量研究，通过主动网络望远镜收集流量，分析攻击活动的时空分布、扫描行为特征和攻击基础设施。


<details>
  <summary>Details</summary>
Motivation: 服务器端组件化Web框架的普及引入了新的应用层攻击面，但缺乏互联网规模的实证理解。React2Shell漏洞（CVSS 10.0）公开披露后，虽然观察到野外利用，但缺乏对其互联网规模利用活动的系统研究。

Method: 使用主动网络望远镜收集流量，开发确定性检测方法识别针对React Server Components端点的攻击尝试，分析攻击流量的时间演化、地理和自治系统分布、扫描行为特征，并检查攻击载荷以理解攻击基础设施和交付机制。

Result: 发现披露后快速出现的攻击活动，呈现自动化扫描活动模式、地理分布的扫描器、集中的后端基础设施。首次量化表征了React2Shell触发的扫描活动，包括不同扫描器的数量、地理和自治系统分布、参与攻击尝试的后端基础设施规模。

Conclusion: 本研究首次对React2Shell漏洞的互联网规模利用活动进行了系统测量，揭示了攻击活动的特征模式，为理解服务器端组件框架的安全威胁提供了实证基础。
...
TL;DR: 提出基于预期损失分解的复合关键风险指标(KRI)，整合威胁、影响和暴露三个维度，相比CVSS和EPSS能更好地将修复工作与实际风险降低对齐。


<details>
  <summary>Details</summary>
Motivation: 组织普遍使用CVSS严重性评分来优先处理漏洞修复，但CVSS在真实世界漏洞利用数据上的表现接近随机水平(AUPRC仅0.011)，无法有效指导风险降低。

Method: 提出复合关键风险指标(KRI)框架，基于预期损失分解，整合威胁(利用可能性)、影响(严重性)和暴露(资产暴露程度)三个维度。使用包含280,694个CVE的综合数据集，在已知被利用漏洞(KEV)目录上进行评估。

Result: KRI在ROC-AUC上达到0.927，AUPRC达到0.223，显著优于CVSS的0.747和0.011。消融分析显示EPSS单独AUPRC为0.365，但KRI在影响加权修复价值捕获上表现更好：在k=500时捕获92.3%的价值(EPSS为82.6%)，并发现1.75倍的关键严重性被利用CVE。

Conclusion: EPSS在原始漏洞利用检测方面表现更好，但KRI框架通过重新排序考虑影响和暴露，当严重性溢价超过2时净效益超过EPSS，是组织将修复工作与实际风险降低对齐的更好选择。
...
### [3] [Operationalising Cyber Risk Management Using AI: Connecting Cyber Incidents to MITRE ATT&CK Techniques, Security Controls, and Metrics](https://arxiv.org/abs/2603.12455)
*Emad Sherif,Iryna Yevseyeva,Vitor Basto-Fernandes,Allan Cook*

Main category: cs.CR

TL;DR: 提出一个利用NLP自动将网络事件映射到攻击者技术的框架，包含Cyber Catalog知识库和微调的句子转换模型，显著提高了事件与技术之间的语义相似性匹配精度。


<details>
  <summary>Details</summary>
Motivation: 网络攻击频率上升给组织带来挑战，特别是资源有限的小型企业缺乏专业知识、知识和财务资源，需要自动化工具来连接威胁情报与可操作控制措施。

Method: 1) 创建Cyber Catalog知识库，整合CIS关键安全控制、MITRE ATT&CK技术和SMART指标；2) 在74,986个事件-技术对上微调all-mpnet-base-v2模型，增强网络事件与MITRE ATT&CK技术之间的语义相似性。

Result: 微调模型获得Spearman相关系数0.7894和Pearson相关系数0.8756，优于基线模型；预测误差显著降低(MAE=0.135, MSE=0.027)；公开提供Cyber Catalog、训练数据集、训练模型和实现代码。

Conclusion: 该工作填补了威胁情报与运营安全管理之间的空白，为系统化网络事件响应和基于证据的网络风险管理提供了可操作工具，特别适用于资源受限环境。
...
### [4] [Hunting CUDA Bugs at Scale with cuFuzz](https://arxiv.org/abs/2603.12485)
*Mohamed Tarek Ibn ziad,Christos Kozyrakis*

Main category: cs.CR

TL;DR: cuFuzz是首个实用的CUDA导向模糊测试工具，通过解决GPU模糊测试的三个主要障碍，发现了43个未知bug


<details>
  <summary>Details</summary>
Motivation: GPU在现代软件中日益重要，但其异构执行模型和复杂软件栈容易产生内存安全和并发bug，而现有模糊测试方法在GPU上应用不足，存在三个主要障碍

Method: 采用全程序模糊测试避免假阳性；使用NVBit插桩设备端指令并与主机端覆盖率合并；将检测与覆盖率收集解耦，在不同进程中执行主机和设备端检测器

Result: 在14个CUDA程序中发现了43个未知bug（其中19个在商业库中），包括非法内存访问、未初始化读取和数据竞争；相比基线方法发现了更多边和独特输入

Conclusion: cuFuzz是GPU测试工具箱中一个有效且可部署的补充工具，通过解决GPU模糊测试的主要障碍使其变得实用
...
### [5] [Keys on Doormats: Exposed API Credentials on the Web](https://arxiv.org/abs/2603.12498)
*Nurullah Demir,Yash Vekaria,Georgios Smaragdakis,Zakir Durumeric*

Main category: cs.CR

TL;DR: 研究分析了1000万个网页，发现API凭证在网络上广泛暴露，包括全球银行和固件开发商的网站，识别出1748个不同凭证，暴露时间从数月到数年不等。


<details>
  <summary>Details</summary>
Motivation: API凭证的安全处理至关重要，但之前的研究主要关注云平台和GitHub等环境，网络环境中的凭证暴露问题尚未被充分探索。

Method: 通过分析1000万个网页，识别API凭证暴露情况，包括对存档数据的分析以确定暴露持续时间，并分析网络特有的暴露向量和根本原因。

Result: 发现API凭证在网络上广泛公开暴露，识别出1748个来自14个服务提供商的独特凭证，涉及近10000个网页，暴露时间长达数月到数年，大部分源于JavaScript环境。

Conclusion: 网络环境中存在严重的API凭证暴露问题，通过负责任披露可以显著减少凭证暴露，需要加强开发人员对凭证安全处理的教育和实践。
...
### [6] [AEGIS: No Tool Call Left Unchecked -- A Pre-Execution Firewall and Audit Layer for AI Agents](https://arxiv.org/abs/2603.12621)
*Aojie Yuan,Zhiyuan Su,Yue Zhao*

Main category: cs.CR

TL;DR: AEGIS是一个AI代理工具调用的预执行防火墙和审计层，通过深度字符串提取、内容风险扫描和可组合策略验证三阶段管道，在工具执行前进行风险检测和拦截。


<details>
  <summary>Details</summary>
Motivation: 当前AI代理框架缺乏框架无关的控制点，模型生成的工具调用直接传递给执行层，无法在执行前阻止可能产生副作用的风险操作。现有的事后观察性无法在副作用发生前停止这些操作。

Method: AEGIS采用三阶段管道：1) 从工具参数中深度提取字符串；2) 内容优先的风险扫描；3) 可组合的策略验证。高风险调用可暂停等待人工批准，所有决策记录在基于Ed25519签名和SHA-256哈希链的防篡改审计追踪中。

Result: 在48个攻击实例测试中，AEGIS成功在执行前拦截所有攻击；在500个良性工具调用中，误报率为1.2%；在1000次连续拦截中，中位延迟增加8.3毫秒。支持Python、JavaScript和Go的14个代理框架。

Conclusion: AI代理的预执行仲裁机制是实用、低开销且可直接部署的，AEGIS为AI代理工具调用提供了有效的安全防护和审计能力。
...
### [7] [ExpanderGraph-128: A Novel Graph-Theoretic Block Cipher with Formal Security Analysis and Hardware Implementation](https://arxiv.org/abs/2603.12637)
*W. A. Susantha Wijesinghe*

Main category: cs.CR

TL;DR: EGC128是一种基于扩展图交互网络的128位轻量级分组密码，采用20轮平衡Feistel结构，通过稀疏结构连接而非复杂组件实现安全性，在FPGA和ARM平台上均表现出高效性能。


<details>
  <summary>Details</summary>
Motivation: 传统轻量级密码设计主要关注组件代数复杂性的渐进优化，本文提出基于扩展图交互网络的新方法，通过稀疏结构连接性而非组件复杂性来实现扩散和安全性。

Method: 提出ExpanderGraph-128 (EGC128)密码，采用20轮平衡Feistel网络，每轮应用由3-正则扩展图控制的64位非线性变换，图中顶点在局部邻域执行相同的4输入布尔函数。使用MILP进行差分和线性分析。

Result: 差分安全性达147.3位（10轮），全轮保守外推413位；线性分析边界≥2¹⁴⁵；无相关密钥攻击自由轮。FPGA实现261Mbps@100MHz仅需380LUTs；ARM Cortex-M4F软件需25.8KB Flash，加密时间1.66ms。

Conclusion: 扩展图驱动的扩散为轻量级密码学提供了有前景的设计方法，在保持高安全性的同时实现了实际效率，证明了结构连接性而非组件复杂性的有效性。
...
### [8] [Uncovering Security Threats and Architecting Defenses in Autonomous Agents: A Case Study of OpenClaw](https://arxiv.org/abs/2603.12644)
*Zonghao Ying,Xiao Yang,Siyang Wu,Yumeng Song,Yang Qu,Hainan Li,Tianlin Li,Jiakai Wang,Aishan Liu,Xianglong Liu*

Main category: cs.CR

TL;DR: 论文分析了OpenClaw等自主工具调用AI代理带来的新型安全威胁，提出了三层风险分类法和全生命周期代理安全架构（FASA），并介绍了ClawGuard工程实现项目。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型发展为自主工具调用代理（如OpenClaw）带来了操作系统级权限和复杂工作流执行能力，这创造了前所未有的安全挑战，传统内容过滤防御已过时，需要新的安全框架。

Method: 1. 对OpenClaw生态系统进行系统性安全分析；2. 提出三层风险分类法（AI认知层、软件执行层、信息系统层）；3. 设计全生命周期代理安全架构（FASA），包括零信任代理执行、动态意图验证和跨层推理-行动关联；4. 实施ClawGuard工程项目来实践FASA理念。

Result: 识别了OpenClaw的关键漏洞：提示注入驱动的远程代码执行、顺序工具攻击链、上下文遗忘和供应链污染。提出了系统性的安全分类和防御架构，并通过开源项目提供代码和数据集。

Conclusion: 自主AI代理需要全新的安全范式。提出的三层风险分类法和FASA架构为解决这些系统性架构缺陷提供了理论蓝图，ClawGuard项目旨在将高风险实验性工具转变为可信系统，推动该领域的安全发展。
...
### [9] [Why Neural Structural Obfuscation Can't Kill White-Box Watermarks for Good!](https://arxiv.org/abs/2603.12679)
*Yanna Jiang,Guangsheng Yu,Qingyuan Yu,Yi Chen,Qin Wang*

Main category: cs.CR

TL;DR: 提出Canon框架，能够完全恢复被NSO攻击破坏的白盒水印验证，实现100%恢复成功率


<details>
  <summary>Details</summary>
Motivation: NSO攻击通过注入虚拟神经元并保持功能等效，严重破坏了现有白盒水印方案的可靠性，需要解决这一根本性挑战

Method: 将NSO重新定义为生产者-消费者范式中的图一致性威胁模型，基于信号传播的一致性约束，通过探测攻击模型识别冗余/虚拟通道，然后全局规范化网络，重写所有下游消费者以同步布局

Result: 即使在强组合和扩展的NSO攻击下，Canon也能实现100%的恢复成功率，恢复水印可验证性同时保持任务效用

Conclusion: Canon框架首次完全解决了NSO攻击造成的破坏，重新确立了白盒水印方案的可靠性，为对抗结构混淆攻击提供了有效解决方案
...
### [10] [Colluding LoRA: A Composite Attack on LLM Safety Alignment](https://arxiv.org/abs/2603.12681)
*Sihao Ding*

Main category: cs.CR

TL;DR: CoLoRA是一种针对LoRA适配器的组合攻击，单个适配器看似良性，但线性组合后会破坏模型安全性，导致广泛拒绝抑制


<details>
  <summary>Details</summary>
Motivation: 当前防御系统存在组合盲点，无法有效检测多个适配器组合后的恶意行为，需要开发组合感知的防御机制

Method: 通过设计看似良性的LoRA适配器，使它们在单独使用时表现正常，但线性组合后会导致模型对齐退化，无需对抗性提示即可响应有害请求

Result: 在多个开源LLM上，CoLoRA适配器单独表现良性，但组合后攻击成功率很高，表明当前单模块验证方法存在严重安全漏洞
...
### [11] [FoSAM: Forward Secret Messaging in Ad-Hoc Networks](https://arxiv.org/abs/2603.12871)
*Daniel Schadt,Christoph Coijanovic,Thorsten Strufe*

Main category: cs.CR

TL;DR: FoSAM是首个在不可靠ad-hoc网络中提供可证明匿名和前向保密消息传输的协议，无需交互握手，仅需接收方公钥。


<details>
  <summary>Details</summary>
Motivation: 现有应用如Firechat和Bridgefy在抗议活动中被使用，但缺乏前向保密性，被捕抗议者的设备会泄露所有历史消息，带来巨大风险。

Method: 设计FoSAM协议，仅需接收方公钥而非交互握手；通过大规模模拟评估不同用户移动模式下的性能；实现Android原型。

Result: FoSAM在模拟中实现92%-99%的消息投递成功率，是首个在不可靠ad-hoc网络中提供可证明匿名和前向保密消息的协议。

Conclusion: FoSAM解决了现有ad-hoc通信应用缺乏前向保密性的关键安全问题，为抗议等高风险场景提供了更安全的通信方案。
...
### [12] [Almost-Free Queue Jumping for Prior Inputs in Private Neural Inference](https://arxiv.org/abs/2603.12946)
*Qiao Zhang,Minghui Xu,Tingchuang Zhang,Xiuzhen Cheng*

Main category: cs.CR

TL;DR: PrivQJ框架在隐私保护机器学习服务中实现了高效的队列插队机制，通过共享计算和槽位回收技术，在不降低整体性能的情况下优先处理紧急请求，相比现有系统减少了超过一个数量级的开销。


<details>
  <summary>Details</summary>
Motivation: 现有隐私保护机器学习服务框架虽然通过混合密码原语提高了推理效率，但采用顺序批处理方式，缺乏对紧急请求的优先级处理能力。简单的队列插队方法会带来显著的计算和通信开销，增加队列中其他输入的延迟。

Method: 提出PrivQJ框架，利用跨输入的共享计算，通过处理中槽位回收技术，让之前的输入能够"搭便车"到正在进行的批计算中，几乎不增加额外的密码学成本。

Result: 理论分析和实验结果表明，相比最先进的PP-MLaaS系统，PrivQJ实现了超过一个数量级的开销减少，能够在保护隐私的同时高效处理优先级请求。

Conclusion: PrivQJ首次在批处理推理中实现了隐私保护的队列插队机制，通过创新的共享计算策略，在保持系统整体性能的同时，显著降低了优先级处理的开销，为实时隐私保护机器学习服务提供了实用解决方案。
...
### [13] [A Requirement-Based Framework for Engineering Adaptive Authentication](https://arxiv.org/abs/2603.12968)
*Alzubair Hassan,Alkabashi Alnour,Bashar Nuseibeh,Liliana Pasquale*

Main category: cs.CR

TL;DR: 提出一个用于工程化自适应认证系统的框架，通过上下文目标模型和扩展特征模型来动态选择有效的认证方法，以应对物联网、车联网等动态环境中的安全风险变化。


<details>
  <summary>Details</summary>
Motivation: 在物联网、车联网、医疗保健和智慧城市等动态环境中，安全风险会随着上下文因素（如用户身份、位置、设备类型）的变化而变化。现有的自适应认证系统在以下方面存在不足：(a) 表示上下文因素、需求和认证方法；(b) 理解上下文因素和认证方法对需求满足的影响；(c) 选择既能降低安全风险又能最大化满足需求的有效认证方法。

Method: 提出一个框架，包含：1) 上下文目标模型来表示需求和上下文因素对安全风险及需求优先级的影响；2) 扩展特征模型来表示潜在认证方法及其对缓解安全风险和满足需求的影响；3) 运行时使用基于Z3 SMT求解器编码的模糊因果网络来分析目标模型和特征模型，从而选择有效的认证方法。

Result: 通过在车联网和医疗保健领域的实际认证场景中应用该框架，进行了演示和评估，验证了其有效性。

Conclusion: 该框架为工程化自适应认证系统提供了一种系统化方法，能够动态选择有效的认证方法来应对上下文因素和安全风险的变化，弥补了现有系统的不足。
...
### [14] [Mitigating Collusion in Proofs of Liabilities](https://arxiv.org/abs/2603.12990)
*Malcom Mohamed,Ghassan Karame*

Main category: cs.CR

TL;DR: 提出了一种新的许可证明负债模型，通过无需用户合作即可检测不诚实服务商行为，解决了现有方案在服务商与用户共谋攻击下的安全缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有加密货币交易所的负债证明方案存在设计缺陷，无法抵抗服务商与现有用户共谋的现实攻击场景，需要更安全的方案来增强用户信任。

Method: 提出许可证明负债模型，核心是新的许可向量承诺原语，结合KZG承诺的同态特性和BLS签名，确保承诺向量只包含用户明确签名的值。

Result: 原型实现表明，在提供更强安全性的同时，服务器性能相比现有方案提升高达10倍。

Conclusion: 许可证明负债模型解决了现有负债证明方案的安全缺陷，在增强安全性的同时提升了性能，为加密货币交易所提供了更可靠的信任机制。
...
### [15] [FraudFox: Adaptable Fraud Detection in the Real World](https://arxiv.org/abs/2603.13014)
*Matthew Butler,Yi Fan,Christos Faloutsos*

Main category: cs.CR

TL;DR: FraudFox是一个在资源受限环境中对抗欺诈攻击的系统，通过动态权重调整和最优决策面计算，已在亚马逊生产环境中部署并取得显著效果。


<details>
  <summary>Details</summary>
Motivation: 解决资源受限环境下的欺诈检测问题，需要处理多个风险评估模块的分数融合，以及在业务限制（如最多调查次数或最大欺诈损失）下做出最优决策。欺诈者会随系统决策而改变行为，因此系统需要具备适应性。

Method: 1. 使用扩展卡尔曼滤波器（EKF）与动态重要性权重，自动持续更新各风险评估模块的权重；2. 推导最优决策面并计算帕累托最优集，支持"假设分析"；3. 系统具备适应性，能应对欺诈者行为变化。

Result: FraudFox系统具有可扩展性、适应性、有效性，已在亚马逊生产环境中部署，作为欺诈预防子系统的增强组件，带来了显著的性能提升。

Conclusion: FraudFox成功解决了资源受限环境下的欺诈检测问题，通过动态权重调整和最优决策制定，实现了在业务限制下的高效欺诈预防，并能适应欺诈者的行为变化。
...
### [16] [Purify Once, Edit Freely: Breaking Image Protections under Model Mismatch](https://arxiv.org/abs/2603.13028)
*Qichen Zhao,Shengfang Zhai,Xinjian Bai,Qingni Shen,Qiqi Lin,Yansong Gao,Zhonghai Wu*

Main category: cs.CR

TL;DR: 论文提出了一种后发布净化框架，评估图像保护在模型不匹配情况下的生存能力，并设计了两种无需访问保护图像或防御内部信息的净化器，能有效恢复被保护图像的编辑性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能实现高质量图像编辑，但可能被滥用于未经授权的风格模仿和有害内容生成。现有的主动图像保护方法在真实后发布场景中存在局限性：内容所有者无法控制下游处理流程，且针对代理模型优化的保护措施在攻击者使用不匹配的扩散管道时可能失效。

Method: 提出了统一的后发布净化框架来评估保护生存性，并设计了两种净化器：1) VAE-Trans：通过潜在空间投影校正受保护图像；2) EditorClean：使用扩散变换器进行指令引导重建，利用架构异质性。两种方法都无需访问受保护图像或防御内部信息。

Result: 在2100个编辑任务和六种代表性保护方法上，EditorClean能持续恢复编辑性。相比受保护输入，PSNR提高3-6 dB，FID降低50-70%；相比先前净化基线，PSNR提高约2 dB，FID降低30%。

Conclusion: 研究揭示了"一次净化，自由编辑"的失效模式：一旦净化成功，保护信号基本被移除，实现无限制编辑。这强调了需要在模型不匹配情况下评估保护措施，并设计对异构攻击者具有鲁棒性的防御机制。
...
### [17] [Defensible Design for OpenClaw: Securing Autonomous Tool-Invoking Agents](https://arxiv.org/abs/2603.13151)
*Zongwei Li,Wenkai Li,Xiaoqi Li*

Main category: cs.CR

TL;DR: 论文提出OpenClaw等AI代理存在安全架构缺陷，需要从软件工程角度系统性解决，而非仅修补漏洞。作者提出了防御性设计蓝图、风险分类、安全工程原则和研究议程。


<details>
  <summary>Details</summary>
Motivation: OpenClaw类代理虽然能提高生产力，但默认不安全，因为它们将不受信任的输入、自主行动、可扩展性和特权系统访问结合在单一执行循环中。这代表了一类在真实操作环境中与接口交互、操作文件、调用工具和安装扩展的代理，其安全应被视为软件工程问题而非特定产品问题。

Method: 提出了防御性设计的蓝图，包括风险分类法、安全工程原则和实际研究议程，旨在将安全制度化到代理构建中。

Result: 论文为AI代理安全提供了系统性框架，将社区关注点从孤立漏洞修补转向系统性防御工程和稳健部署实践。

Conclusion: AI代理安全需要架构层面的系统性解决方案，通过防御性设计原则、风险分类和研究议程来建立安全工程实践，确保代理在真实环境中的安全部署。
...
### [18] [Verification of Robust Properties for Access Control Policies](https://arxiv.org/abs/2603.13181)
*Alexander V. Gheorghiu*

Main category: cs.CR

TL;DR: 提出"鲁棒性属性验证"概念，用于验证访问控制策略的结构性承诺，无论未决决策如何解决或后续扩展如何


<details>
  <summary>Details</summary>
Motivation: 现有访问控制策略验证方法要求策略完整且完全确定，但实践中策略是迭代开发、由独立维护组件组合而成，并随着组织结构演变而扩展

Method: 定义支持判断$\Vdash_{P}φ$表示策略$P$具有鲁棒属性$φ$，包含蕴含、合取、析取和否定连接词；证明其组合性（通过单调性定理）；将判断简化为二阶逻辑编程语言中的证明搜索

Result: 建立了这种简化的可靠性和完备性，产生了用于鲁棒安全属性的有限且可执行的验证程序

Conclusion: 鲁棒属性验证解决了实践中策略迭代开发和扩展的验证问题，提供了形式化的验证框架和可执行程序
